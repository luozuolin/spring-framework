...
final class JdkDynamicAopProxy implements AopProxy, InvocationHandler, Serializable {
	...
	public Object invoke(Object proxy, Method method, Object[] args) throws Throwable {
		...
		try {
			...
			// 2、处理hashCode方法，如果接口中没有定义hashCode而在实现类中
			// 覆盖了hashCode方法，那么该hashCode方法不会被增强
			else if (!this.hashCodeDefined && AopUtils.isHashCodeMethod(method)) {
				// The target does not implement the hashCode() method itself.
				return hashCode();
			}
			// 3、如果目标对象是DecoratingProxy类型，则返回目标对象的最终对象类型
			// DecoratingProxy接口只有一个getDecoratedClass方法，用于返回目标对象
			// 的最终对象类型
			...
			// 例如：Class1.isAssignableFrom(Class2)
			...
			//如果拦截器链为空，我们就直接通过反射调用目标对象的方法，避免去创建MethodInvocation
